...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_coords, supp_imgs, fore_mask, back_mask, qry_coords, qry_imgs):
        """
        Args:
        ...
        supp_pts = sum([sum([shot.shape[1] for shot in way]) for way in supp_coords])

        ###### Extract features ######
        coords_concat = torch.cat([torch.cat(way, dim=1) for way in supp_coords]
                                + [torch.cat(qry_coords, dim=1),], dim=1)[0]

        imgs_concat = torch.cat([torch.cat(way, dim=1) for way in supp_imgs]
                                + [torch.cat(qry_imgs, dim=1),], dim=1)[0]
        img_fts = self.encoder([coords_concat, imgs_concat])

        fts_size = img_fts.shape[-1]
        supp_fts = img_fts[:supp_pts].view(
            n_ways, n_shots, batch_size, -1, fts_size)
        qry_fts = img_fts[supp_pts:].view(
            n_ways, n_shots, batch_size, -1, fts_size)
    ...
    def calDist(self, fts, prototype, scaler=20):
        ...
        Args:
            ...
            prototype: prototype of one semantic class
                expect shape: 1 x C
        """
        dist = F.cosine_similarity(fts, prototype[None, None, ...], dim=3) * scaler
        return dist
    ...
    def getFeatures(self, fts, mask):
        ...
        masked_fts = torch.sum(fts * mask[..., None], dim=1) \
            / (mask[..., None].sum(dim=1) + 1e-5) # 1 x C
        return masked_fts
    ...
    def alignLoss(self, qry_fts, pred, supp_fts, fore_mask, back_mask):
        ...
        qry_prototypes = torch.sum(qry_fts.permute(0,1,3,2).unsqueeze(1) * pred_mask, dim=(0, 2, 4))
        qry_prototypes = qry_prototypes / (pred_mask.sum((0, 2, 4)) + 1e-5)  # (1 + Wa) x C
        ...
        for way in range(n_ways):
            ...
            for shot in range(n_shots):
                ...
                loss = loss + F.cross_entropy(
                    supp_pred, supp_label[None, None,...], ignore_index=255) / n_shots / n_ways
        return loss
