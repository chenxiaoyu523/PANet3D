...
from models.fewshot3d import FewShotSeg
from dataloaders.customized import voc_fewshot, coco_fewshot, scannet_fewshot
    ...
    with torch.no_grad():
        for run in range(_config['n_runs']):
            ...
            for sample_batched in tqdm.tqdm(testloader):
                ...
                query_coords = [query_image.cuda()
                                for query_image in sample_batched['query_coords']]
                query_images = [query_image.cuda()
                                for query_image in sample_batched['query_images']]
                query_labels = torch.cat(
                    [query_label.cuda()for query_label in sample_batched['query_labels']], dim=0)

                query_pred, _ = model(support_coords, support_images, support_fg_mask, support_bg_mask,
